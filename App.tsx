...
const App: React.FC = () => {
  ...
  useEffect(() => {
    console.log("App.tsx: Component mounted, checking DOM...");
    const root = document.getElementById("root");
    if (root) {
      console.log(
        "App.tsx: Root element found, children count:",
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      {isProcessing && (
        <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md transition-all duration-500 animate-in fade-in">
          <div className="relative w-16 h-16 mb-6">
            ...
            {/* Center Star */}
            <div className="absolute inset-0 flex items-center justify-center text-[#d4af37] text-xl animate-pulse">
              ✦
            </div>
          </div>
      ...
      <div
        className={`absolute top-[5%] left-0 w-full flex justify-center pointer-events-none z-0 transition-opacity duration-700 ${
          isSignatureOpen ? "opacity-0" : "opacity-100"
        }`}
      >
        <h1
          ...
          style={{
            ...
            background:
              "linear-gradient(to bottom, #ffffff 20%, #e8e8e8 50%, #b0b0b0 90%)",
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
            // 3D Depth Shadows + Glow
            filter:
              "drop-shadow(0px 5px 5px rgba(0,0,0,0.8)) drop-shadow(0px 0px 20px rgba(255,255,255,0.4))",
          }}
        >
          Merry Christmas
        </h1>
      ...
      {/* PHOTO VIEWER MODAL (Pinch and Pull Gesture) */}
      {isPhotoViewerOpen && viewerPhotoUrl && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md transition-opacity duration-300 animate-in fade-in">
          <div className="relative w-full h-full max-w-4xl max-h-[90vh] flex items-center justify-center">
            {/* Close Button */}
            <button
              onClick={() => setIsPhotoViewerOpen(false)}
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/80 border border-white/30 text-white flex items-center justify-center hover:bg-white hover:text-black transition-colors z-50 text-xl"
            >
              ×
            </button>
      ...
      <div
        className={`absolute top-6 right-6 md:top-10 md:right-10 z-30 pointer-events-auto flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-4 transition-opacity duration-500 ${
          isSignatureOpen || isProcessing
          ...
          {showCamera ? (
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M12 18.75H4.5a2.25 2.25 0 01-2.25-2.25V9m12.841 9.091L16.5 19.5m-1.409-1.409c.407-.407.659-.97.659-1.591v-9a2.25 2.25 0 00-2.25-2.25h-9c-.621 0-1.184.252-1.591.659m3.75-3.75l3.75-3.75"
              />
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      <div
        className={`absolute bottom-6 left-6 z-20 pointer-events-none transition-opacity duration-500 ${
          isSignatureOpen ? "opacity-0" : "opacity-100"
        }`}
      >
